# example_scripts/run_cifar10.py
...
# make sure base/ is importable
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), ".")))

# modular package
from base.population import Population
from base.structures import Datasets
from base.structures import ModuleComposition  # noqa
...
def _pick_val_metric_name():
    """Choose the correct val_accuracy metric name."""
    return "val_accuracy"
...
def run_cifar10_full(
    ...
    base_ds = Datasets(training=[x_train, y_train], test=[x_test, y_test])
    base_ds.SAMPLE_SIZE = 20000
    base_ds.TEST_SAMPLE_SIZE = 2000
    base_ds.custom_fit_args = None

    improved_ds = Datasets(training=[x_train, y_train], test=[x_test, y_test])
    improved_ds.custom_fit_args = {
        ...
        "callbacks": _build_callbacks("training.csv", "best_model_checkpoint.h5"),
    }
    ...
    logging.info("Hi, this is a test run.")

    compiler = {"loss": "categorical_crossentropy", "metrics": ["accuracy"]}

    # --- build populations ---
    populations = []
    for _ in range(num_populations):
        pop = Population(base_ds, input_shape=x_train.shape[1:],
                         population_size=population_size, compiler=compiler)

        pop.module_share_mode = share_mode
        pop.create_module_population(module_population_size, global_configs,
                                     possible_components,
                                     possible_complementary_components)
        ...
        # periodic bursts (MAGIC-T+)
        if hasattr(pop, "magic_t_burst_every"):
            pop.magic_t_burst_every = 5
            pop.magic_t_burst_min = 3
            pop.magic_t_burst_max = 5

        # conflict logging
        if hasattr(pop, "conflict_log_every"):
            pop.conflict_log_every = 3
        if hasattr(pop, "conflict_warn_threshold_pct"):
            pop.conflict_warn_threshold_pct = 35.0
    ...
    for i, population in enumerate(populations):
        ...
        print("Using data augmentation for final training.")
        improved_ds.custom_fit_args["epochs"] = final_model_training_epochs
...
# ---------------------------
def parse_args():
    ap = argparse.ArgumentParser(description="CoDeepNEAT runner with live weight sharing & MAGIC-T toggles")
    ...
    ap.add_argument("--max-steps-per-epoch", type=int, default=15)
